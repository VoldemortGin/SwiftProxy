...
public actor SSLHandler {
    ...
    private func verifyServerTrust(
        ...
        switch trustPolicy {
        case .default:
            return await verifyDefaultTrust(trust: trust, host: host)

        case .pinned:
            return await verifyPinnedCertificate(certificates: certificates, host: host)

        case .custom(let validator):
            return await validator(certificates, host)

        case .allowAll:
            os_log(.default, log: logger, "Allowing all certificates (insecure)")
            return true
        }
    }

    private func verifyDefaultTrust(trust: sec_trust_t, host: String) async -> Bool {
