...
public final class ConfigurationService: ConfigurationServiceProtocol {
    ...
    public func saveConfiguration(_ configuration: ProxyConfiguration) async throws {
        ...
        try await stateQueue.sync {
            ...
            cachedConfigurations[configuration.id] = configuration

            // Save password to keychain if present
            if let password = configuration.password {
                try keychain.passwords.set(password, for: configuration.id.uuidString)
            }
            ...
            os_log(.debug, log: logger, "Configuration saved: %@", configuration.id.uuidString)
        }
    }
    ...
    public func deleteConfiguration(id: UUID) async throws {
        ...
        try await stateQueue.sync {
            // Remove from cache
            guard cachedConfigurations.removeValue(forKey: id) != nil else {
                throw AppError.proxyConfigurationInvalid("Configuration not found")
            }

            // Delete password from keychain
            try? keychain.passwords.delete(for: id.uuidString)
            ...
            await MainActor.run {
                ...
                if activeConfigurationSubject.value?.id == id {
                    activeConfigurationSubject.send(nil)
                    UserDefaults.standard.removeObject(forKey: activeConfigIDKey)
                }
            }

            os_log(.debug, log: logger, "Configuration deleted: %@", id.uuidString)
        }
    }
    ...
    public func importConfigurations(from data: Data, merge: Bool) async throws {
        ...
        for var config in configs {
            ...
            if let password = try? keychain.passwords.get(for: config.id.uuidString) {
    ...
    public func validateConfiguration(_ configuration: ProxyConfiguration) async -> ProxyConfiguration.ValidationResult {
        ...
        if case .valid = result {
            ...
            if duplicateName {
                os_log(.warning, log: logger, "Duplicate configuration name: %@", configuration.name)
    ...
    private func loadConfigurationsSync() throws -> [ProxyConfiguration] {
        ...
        for index in configs.indices {
            if let password = try? keychain.passwords.get(for: configs[index].id.uuidString) {
