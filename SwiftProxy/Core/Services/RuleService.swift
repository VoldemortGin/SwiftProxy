...
public final class RuleService: RuleServiceProtocol {
    ...
    public func saveRule(_ rule: ProxyRule) async throws {
        ...
        try await stateQueue.sync {
            ...
            os_log(.debug, log: logger, "Rule saved: %@", rule.id.uuidString)
        }
    }
    ...
    public func deleteRule(id: UUID) async throws {
        ...
        try await stateQueue.sync {
            // Remove from cache
            guard cachedRules.removeValue(forKey: id) != nil else {
            ...
            os_log(.debug, log: logger, "Rule deleted: %@", id.uuidString)
        }
    }
    ...
    public func toggleRule(id: UUID) async throws {
        ...
        os_log(.info, log: logger, "Toggling rule: %@ (enabled: %@)", rule.name, (!rule.isEnabled).description)
    ...
    public func duplicateRule(id: UUID) async throws -> ProxyRule {
        ...
        let duplicate = ProxyRule(
            ...
            processFilter: original.processFilter
        )
    ...
    private func loadRulesSync() throws -> [ProxyRule] {
        ...
        sortedEnabledRules = rules.filter { $0.isEnabled }.sorted()
    ...
    private func defaultRules() -> [ProxyRule] {
        [
            ProxyRule(
                ...
                action: .direct,
                priority: 100,
                description: "Bypass proxy for local network"
            ),
            ProxyRule(
                name: "Localhost",
                pattern: "127.0.0.1",
                ...
                action: .direct,
                priority: 99,
                description: "Bypass proxy for localhost"
            ),
            ProxyRule(
                name: "Default Proxy",
                pattern: ".*",
                ...
                action: .proxy,
                priority: 0,
                description: "Default rule: use proxy for all traffic"
            )
        ]
    }
}
