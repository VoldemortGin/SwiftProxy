...
extension ProxyConfiguration {
    #if os(macOS)
    /// Convert to SystemConfiguration proxy dictionary (macOS only)
    ...
    public func toURLSessionProxyDict() -> [AnyHashable: Any] {
        ...
        #else
        // On iOS/tvOS/watchOS, use string keys as fallback
        switch type {
        case .http:
            dict["HTTPEnable"] = 1
            dict["HTTPProxy"] = host
        ...
        if !bypassDomains.isEmpty {
            dict["ExceptionsList"] = bypassDomains
        }
        #endif
...
extension ProxyConfiguration {
    enum CodingKeys: String, CodingKey {
        ...
        case notes, createdAt, updatedAt, lastUsed
    }
