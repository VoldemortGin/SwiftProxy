...
public actor TrafficInterceptor {
    ...
    public init(
        ...
        logger: Logger = Logger.networkLog
    ) {
        self.packetHandler = packetHandler
        self.logger = logger
        self.rateLimiter = PerformanceOptimizations.RateLimiter(capacity: 10000, refillRate: 10000)
    }
    ...
    public func startIntercepting() async throws {
        guard !isIntercepting else {
            logger.warning("Traffic interception is already active")
            return
        }

        logger.info("Starting traffic interception")

        isIntercepting = true
        statistics.startTime = Date()

        logger.info("Traffic interception started")
    }

    /// Stop intercepting network traffic
    public func stopIntercepting() async {
        guard isIntercepting else {
            logger.warning("Traffic interception is not active")
            return
        }

        logger.info("Stopping traffic interception")
    ...
    public func interceptPacket(_ packet: Data, protocolFamily: Int32) async throws -> InterceptResult {
        ...
        guard await rateLimiter.allowRequest() else {
            ...
            logger.warning("Packet dropped due to rate limiting")
            return .drop
        }
    ...
    public func interceptFlow(_ flow: NWConnection.State) async {
        ...
        logger.debug("New flow intercepted")
    ...
    private func closeFlow(_ flow: NetworkFlow) async {
        ...
        logger.debug("Flow closed: \(flow.identifier) (duration: \(duration)s, packets: \(flow.packetCount))")
    ...
    private func cleanupStaleFlows() async {
        ...
        if !staleFlows.isEmpty {
            logger.debug("Cleaned up \(staleFlows.count) stale flows")
        }
    }
    ...
    public func addObserver(_ observer: any TrafficObserver) {
        ...
        logger.debug("Added traffic observer")
    }
    ...
    public func removeObserver(_ observer: any TrafficObserver) {
        ...
        logger.debug("Removed traffic observer")
    }
