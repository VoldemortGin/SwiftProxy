...
public final class ProxyService: ProxyServiceProtocol {
    ...
    public func saveConfiguration(_ configuration: ProxyConfiguration) async throws {
        try await stateQueue.sync {
            ...
            var configurations = (try? loadConfigurationsSync()) ?? []
            ...
            if let encoded = try? JSONEncoder().encode(configurations) {
                ...
                os_log(.debug, log: logger, "Saved configuration: %@", configuration.name)
            } else {
            ...
            if let password = configuration.password {
                try savePasswordToKeychain(password, for: configuration.id)
            }
        }
    }
    ...
    public func deleteConfiguration(id: UUID) async throws {
        try await stateQueue.sync {
            ...
            if let encoded = try? JSONEncoder().encode(configurations) {
                ...
                try? deletePasswordFromKeychain(for: id)

                os_log(.debug, log: logger, "Deleted configuration: %@", id.uuidString)
            } else {
                throw AppError.dataEncodingFailed
            }
        }
