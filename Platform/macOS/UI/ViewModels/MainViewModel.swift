...
final class MainViewModel: ObservableObject {
    ...
    /// Clean up caches and non-essential data in response to memory warning
    func cleanupCaches() {
        os_log(.info, log: logger, "Cleaning up caches due to memory pressure")

        // Clear old requests, keeping only the 20 most recent
        if recentRequests.count > 20 {
            recentRequests = Array(recentRequests.prefix(20))
        }

        // Reset error message if it's been displayed
        errorMessage = nil
    }

    /// Close idle connections and perform aggressive cleanup for critical memory pressure
    func closeIdleConnections() async {
        os_log(.info, log: logger, "Closing idle connections due to critical memory pressure")

        // Disable proxy if enabled to free resources
        if isProxyEnabled {
            await disableProxy()
        }

        // Clear all requests except the 5 most recent
